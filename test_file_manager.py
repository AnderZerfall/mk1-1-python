...
SOURCE_LINES = ['Test line 1', 'Test line boo 2', 'Test line foo 3', 'Test line boo 4']
FILTERED = ['Test line boo 2', 'Test line boo 4']
TARGET_FILE = 'public/filtered_test.txt'

FILTER_LINES_TEST_PARAMS = [pytest.param(SOURCE_LINES, 'boo', FILTERED, id='positive'),
                       pytest.param(SOURCE_LINES, 'bruh', None, id='not found'),
                       ]
...
class TestFileManager():
    @pytest.mark.parametrize("source_file,expected_outcome", [(SOURCE_FILE, SOURCE_LINES)])
    ...
    @pytest.mark.parametrize("source_lines,target, expected_outcome", FILTER_LINES_TEST_PARAMS)
    def test_filter_lines(self, source_lines, target, expected_outcome):
        lines = self.manager.filter_lines(source_lines, target)
        assert lines == expected_outcome

    @pytest.mark.parametrize("source_lines,target_file", [(FILTERED, TARGET_FILE)])
